...
import {
  ...
  submitCustomerFeedbackThunk,
  clearFeedbackSuccess,
  fetchCustomerMessages,
  sendCustomerMessageThunk,
  clearMessageSendSuccess,
  fetchCustomerAnalytics,
  fetchCustomerNotifications,
} from '../features/customerPortal/customerPortalSlice';
import { toast } from 'react-toastify';

const CustomerPortalPage: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
  const { profile, orders, loyalty, feedbackSuccess, messages, messageSendSuccess, analytics, notifications } = useSelector((state: RootState) => state.customerPortal);
  const [feedback, setFeedback] = useState('');
  const [message, setMessage] = useState('');

  useEffect(() => {
    dispatch(fetchCustomerProfile());
    dispatch(fetchCustomerOrderHistory());
  ...
  const handleFeedback = async (e: React.FormEvent) => {
    e.preventDefault();
    await dispatch(submitCustomerFeedbackThunk(feedback));
  };

  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    await dispatch(sendCustomerMessageThunk(message));
  };
      ...
      {analytics && (
        <div className="bg-white p-4 rounded shadow mb-8">
          ...
          <div className="flex gap-8 mb-2">
            ...
            <div><b>Total Spent:</b> ${analytics.totalSpent}</div>
